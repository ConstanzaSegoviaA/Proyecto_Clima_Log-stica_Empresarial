...
5. Tercero, abrir la carpeta SQL entrar al archivo `traspasosql.ipynb` y ejecutarlo desde el inicio, ahí realizaremos la creación de las tablas y el traspaso de los datos a SQL.
6. Si ya ejecuto la creación de las tablas en la base de datos, abrir la carpeta SQL y ejecutar el archivo `estudio.sql` para ver los query y crear las tablas en la base de datos de `incidentes.csv` y `UNION.csv` que usaremos para el análisis.
7. Con esos archivos SQL, abrir la carpeta Notebooks en la subcarpeta ANALISIS el archivo `analisis.ipynb` y ejecutarlo desde el inicio ahí realizaremos el análisis de los datos y generaremos los insights para poder hacer el estudio de Hipótesis.
8. Siguiente, abrir la carpeta Notebooks en la subcarpeta HIPOTESIS_KPI el archivo `Hipotesis_KPI.ipynb` y ejecutarlo desde el inicio ahí realizaremos el análisis de las hipótesis y llegamos a las conclusiones del proyecto.
9. Finalizamos el proyecto con la carpeta  Notebooks abrimos el archivo `Dashboard_Logistica.pbix` y ejecutamos el archivo o bien abrir la página web ["Más alla del reloj"](https://app.powerbi.com/groups/me/reports/0f356bfb-4040-43ca-a716-4d53d6f4dc8b/1c4bdc23041da9952e57?experience=power-bi).